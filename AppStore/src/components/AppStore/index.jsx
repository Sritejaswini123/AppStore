...
const AppStore = () => {
  ...
  const getFilteredApps = (searchedApps) => {
    const filteredApps = searchedApps.filter((each) => each.category === activeTabId);
    return filteredApps;
  };

  const searchedApps = getSearchedApps();
  const filteredApps = getFilteredApps(searchedApps);
  ...
  return (
    <div className="container">
      <div className="main">
        ...
        <ul className="tabs">
          {tabsList.map((each) => (
            <TabItem clickTabItem={onClickTab} tabsLiss={each} key={each.tabId} />
          ))}
        </ul>

        <ul className="apps">
          {filteredApps.map((each) => (
            <AppItem appsLiss={each} key={each.appId} />
          ))}
        </ul>
      </div>
    </div>
